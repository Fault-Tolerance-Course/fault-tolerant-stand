...
func NewRegistry(storage *storage.Registry, gateway *gateway.Registry) *Registry {
	return &Registry{
		GetAd:        get_ad.NewService(storage.Ad, gateway.Review),
		GetActiveAds: get_active_ads.NewService(storage.Ad),
		Creation:     create_ad.NewService(storage.Ad),
		Deactivation: deactivate.NewService(storage.Ad, storage.Ad),
	}
}
